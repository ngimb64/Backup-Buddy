...
# Function Index #
########################################################################################################################
CopyFile - Copies file in error validated wrapper.
FileHandler - If file exists check if the source file has a more recent time stamp than the destination file, if so \
              copy the file. If the file does not exist, simply copy the file.
DirHandler - Confirms the directory of the destination path exists. If not, the directory is created to prevent errors.
PrintErr - Prints time controlled error message.
main - Gathers users input and executes file copy operations based on the source and destination path provided.
########################################################################################################################
"""


# Returns time of last modification #
def TimeCheck(filename: str): return datetime.fromtimestamp(os.stat(filename).st_mtime)
...
def main():
    ...
    if prompt == 'r':
        ...
        if os.name == 'nt':
            ...
            re_ext_path = re.compile(r'(?<={0}\\).+$'.format(str(re_edge_path.group(0))))
        else:
            re_edge_path = re.search(r'[^/]{1,255}$', src_path)
            # Insert path edge regex match into regex to match any path past the edge anchor point #
            re_ext_path = re.compile(r'(?<={0}/).+$'.format(str(re_edge_path.group(0))))
        ...
        for dir_path, dir_names, file_names in os.walk(src_path):
            ...
            for dir_name in dir_names:
                ...
                # If the path is part of recursive structure #
                    ...
                    # If the OS is Windows #
                    if os.name == 'nt':
                        dir_path = f'{dest_path}\\{recursive_path}\\{dir_name}'
                    # If the OS is Linux #
                    else:
                        dir_path = f'{dest_path}/{recursive_path}/{dir_name}'
            ...
            for file in file_names:
                ...
                if not recursive_path:
                    ...
                    FileHandler(src_file, dest_file)
                # If the path is part of recursive structure #
                else:
                    # If the OS is Windows #
                    if os.name == 'nt':
                        src_file = f'{dir_path}\\{file}'
                        dest_file = f'{dest_path}\\{recursive_path}\\{file}'
                    # If the OS is Linux #
                    else:
                        src_file = f'{dir_path}/{file}'
                        dest_file = f'{dest_path}/{recursive_path}/{file}'
...
if __name__ == '__main__':
    ...
    # If unknown exception occurs #
        ...
        PrintErr('Unexpected exception occurred .. exiting, check log', 2)
